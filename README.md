...
|11.8.|5|103|Statistics for all time values shown on main screen if Stat-button is pressed. Had to struggle with hooks and rendering. Also the graphs shift if stats is ON or OFF. Not nice...|Fix jumping graphs, and shows statistics for current year and current month|
|14.8.|3|106|Fixed the jumping screen while showing statistics. Had to deal with calling hooks in every render... Statistics still is rather lame. It could show accumulation from the beginning of year, beginning of current month etc. But now it suffices to show total accumulation since start of collecting the data|return course project work, and concentrate on something else for a while :) |
